...
function Item(props: MusicListItem) {
  ...
  return (
    <li className="flex items-center">
      <p
        className="mr-4 lg:ml-4 flex items-center justify-center tooltip font-semibold w-6 h-6 rounded-full bg-white text-black text-xs"
      ...
      <a className="flex-1" href={props.url}>
        <p className="flex flex-col gap-1 items-start">
          <strong className="lg:font-medium block">{props.name}</strong>
          <span
            className="tooltip before:font-semibold hidden lg:block"
      ...
      <div className="hidden lg:flex flex-col gap-1 text-white">
        {props.spotifyId && (
          <a
            className="tooltip before:font-semibold"
...
export default function MusicList(props: MusicListProps) {
  return (
    <div className="px-6 lg:px-20 xl:px-36">
      ...
      {props.musics.length === 0 ? <p>Nenhuma música encontrada</p> : (
        <ul>
          {props.musics.map((music) => <Item key={music.id} {...music} />)}
        </ul>
      )}
    </div>
  );
